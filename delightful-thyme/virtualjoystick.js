// https://github.com/jeromeetienne/virtualjoystick.js/blob/master/virtualjoystick.js
...
const tan_Pi_div_8 = Math.sqrt(2.0) - 1.0;

function getCardinalDirection(dx, dy) {
    const ddx = Math.abs(dx);
    const ddy = Math.abs(dy);
    const d = Math.sqrt(dx * dx + dy * dy);
    if (d < 35) {
        return 'none';
    }
    if (ddx > ddy) {
        if (ddy / ddx <= tan_Pi_div_8) {
            return dx > 0 ? 'right' : 'left';
        }
        return dy > 0 ? dx > 0 ? 'down-right' : 'down-left' : dx > 0 ? 'up-right' : 'up-left';
    } else if (ddy > 0) {
        if (ddx / ddy <= tan_Pi_div_8) {
            return dy > 0 ? 'down' : 'up';
        }
        return dx > 0 ? dy > 0 ? 'down-right' : 'up-left' : dy > 0 ? 'down-left' : 'up-left';
    }
    return 'none';
}
...
{
    const dx = this._stickX - this._baseX;
    const dy = this._stickY - this._baseY;
    
    switch (getCardinalDirection(dx, dy)) {
        case 'up':
